...
export default function TouchPad({ style, ...props }: TouchPadProps) {
  const socket = useSocketStore((s) => s.socket);

  const { moveSensitivity, scrollSensitivity, accelerationThreshold, maxAccelerationFactor } =
    useSettingsStore(
      useShallow((s) => ({
  ...
  const bind = useGestures({
    ...
    onMove(x, y) {
      const [acceleratedX, acceleratedY] = getAcceleratedCoords(
        x,
        y,
    ...
    onScroll(x, y) {
      socket?.send(
        JSON.stringify({
          type: 'scroll',
          x: Math.trunc(x * scrollSensitivity),
          y: Math.trunc(y * scrollSensitivity),
        }),
      );
    },
    onLeftPress() {
